...
public final class VirtualWorld extends PApplet {
    ...
    */
    ...
    public void mousePressed() {
        ...
        if (!world.isOccupied(pressed) && (!world.isOccupied(left) || !world.isOccupied(up) || !world.isOccupied(right) || !world.isOccupied(down))){
            GnomeHut hut = new GnomeHut("", pressed, imageStore.getImageList("gnomehut"), 0, "hut");
            world.setOccupancyCell(pressed, hut);
            GnomeWizard wiz = new GnomeWizard("", left, imageStore.getImageList("gnomewizard"), 0, "gnomewizard", 0.1, 0.5, Gnome.Gnome_Strategy, 2);
            if (!world.isOccupied(left)){
                world.addEntity(wiz);
                wiz.scheduleAction(scheduler, world, imageStore);
                world.addEntity(hut);
            } else if (!world.isOccupied(up)) {
                world.addEntity(wiz);
                wiz.scheduleAction(scheduler, world, imageStore);
                world.addEntity(hut);
                ...
                wiz.scheduleAction(scheduler, world, imageStore);
                world.addEntity(hut);
            } else if (!world.isOccupied(down)) {
                world.addEntity(wiz);
                wiz.scheduleAction(scheduler, world, imageStore);
                world.addEntity(hut);
            }
            else {
                System.out.println("No Space for Wiz!");
            }
        }
        else {
            ...
            if (entityOptional.isPresent()) {
                ...
                if (entity.getKey().equals(Tree.TREE_KEY) || entity.getKey().equals(Sapling.SAPLING_KEY)){
                    System.out.println(entity.getId() + ": " + entity.getKey() + " : " + ((Healthey)entity).getHealth());
                }
                else{
...
//        }
    ...
    public void scheduleActions(WorldModel world, EventScheduler scheduler, ImageStore imageStore) {
        for (Entity entity : world.entities()) {
            if(!entity.getKey().equals(Stump.STUMP_KEY) && !entity.getKey().equals(House.HOUSE_KEY)){
                ((AnimateAble)entity).scheduleAction(scheduler, world, imageStore);
            }
            else if (!entity.getKey().equals(Obstacle.OBSTACLE_KEY) && !entity.getKey().equals(Stump.STUMP_KEY) && !entity.getKey().equals(House.HOUSE_KEY)) {
                ((ActionAble) entity).scheduleAction(scheduler, world, imageStore);
            }
        }
    }
